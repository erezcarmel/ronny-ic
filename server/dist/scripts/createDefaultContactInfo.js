...
async function createDefaultContactInfo() {
    try {
        ...
        const existingEnInfo = await prisma_1.default.contactInfo.findFirst({
            where: { language: 'en' },
        });
        if (!existingEnInfo) {
            await prisma_1.default.contactInfo.create({
                data: {
                    language: 'en',
        ...
        const existingHeInfo = await prisma_1.default.contactInfo.findFirst({
            where: { language: 'he' },
        });
        if (!existingHeInfo) {
            await prisma_1.default.contactInfo.create({
